...
export class DifPresentationExchangeService {
  ...
  public async createPresentationRequest(
      ...
      if (signACPContext) {
        let returned:any = await this.getACPContextSign(agentContext, presentationDefinition, options, context, type)
      ...
      if (signPresentationRequest) {
        ...
        return new W3cPresentationRequest({
          context: context,
          type: type,
  ...
  private async getPresentationRequestSign(agentContext: AgentContext, presentationExchangeRequest: DifPresentationExchangeDefinition, options: any, context: string[], type: string[]) {
    const didRecord = await agentContext.dependencyManager.resolve(DidsApi).getCreatedDids({method: 'web'})
    ...
    if (!verificationMethod) {
      throw new DifPresentationExchangeError(`Cannot create presentation request for credentials without a valid ACP owner`)
    }
    return await this.w3cCredentialService.signPresentationRequest(agentContext, {
      format: ClaimFormat.LdpVc,
      proofType: this.getProofTypeForLdpVc(agentContext, presentationExchangeRequest, verificationMethod), // Actually, we take the valid verification method from the presentation definition itself and check if contained in the verification method of the verifier
  ...
  private async getACPContextSign(agentContext: AgentContext, presentationExchangeRequest: DifPresentationExchangeDefinition, options: any, context: string[], type: string[]) {
    const didRecord = await agentContext.dependencyManager.resolve(DidsApi).getCreatedDids({method: 'web'})
    const ourDID = didRecord[0].didDocument!.id
    const verificationMethod = await this.getVerificationMethodForSubjectId(
        agentContext,
    ...
    if (presentationExchangeRequest.requestACP === undefined) {
      throw new CredoError("Unable to process request with ACP Context undefined")
    }

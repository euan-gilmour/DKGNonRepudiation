...
export class DidExchangeProtocol {
  ...
  public async createRequest(
    ...
    const didDocumentCurrent:any = (await agentContext.dependencyManager.resolve(DidsApi).getCreatedDids({method: 'web'}))[0].didDocument;
    const didlong = didDocumentCurrent.id;
    const message = new DidExchangeRequestMessage({ label, parentThreadId, did: didDocument.id, goal, goalCode, didlong })
    ...
    if (isValidPeerDid(didDocument.id) && getNumAlgoFromPeerDid(didDocument.id) === PeerDidNumAlgo.GenesisDoc) {
      const keyForSignature:string[] = [];
      if (didDocumentCurrent) keyForSignature.push(didDocumentCurrent.verificationMethod[0].publicKeyBase58);
      const didDocAttach = await this.createSignedAttachment(
        agentContext,
        didDocument.toJSON(),
          keyForSignature.length > 0 ? keyForSignature : didDocument.recipientKeys.map((key) => key.publicKeyBase58)
      )
  ...
  public async createResponse(
    ...
    if (numAlgo === PeerDidNumAlgo.GenesisDoc) {
      message.didDoc = await this.createSignedAttachment(
        agentContext,
        didDocument.toJSON(),
        keyForSignature.length > 0 ? keyForSignature : Array.from( // if key:web defined use it
          new Set(
            services
  ...
  private async extractAttachedDidDocument(
    ...
    if (message instanceof DidExchangeRequestMessage) {
      const result: any = await agentContext.dependencyManager.resolve(DidsApi).resolveDidDocument(message.didlong);
      ...
      if (!isValid || !signerKeys.every((key) => base58DidLong == key.publicKeyBase58)) {
        const problemCode = DidExchangeProblemReportReason.RequestNotAccepted
        throw new DidExchangeProblemReportError('DID Document signature is invalid.', {problemCode})
      }
    } else {
      if (!isValid || !signerKeys.every((key) => didDocumentKeysBase58?.includes(key.publicKeyBase58))) {
        const problemCode = DidExchangeProblemReportReason.ResponseNotAccepted
        throw new DidExchangeProblemReportError('DID Document signature is invalid.', { problemCode })
      }
    }

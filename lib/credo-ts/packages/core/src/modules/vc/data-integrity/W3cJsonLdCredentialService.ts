...
import type {
  ...
  W3cJsonLdVerifyPresentationOptions, W3cVerifyWrappedPresentationOptions,
...
import {
  W3cSignWrappedPresentationOptions,
...
export class W3cJsonLdCredentialService {
  ...
  public async signWrappedPresentation(
    ...
    return JsonTransformer.fromJSON(result, W3cWrappedVerifiablePresentation)
  }
  ...
  public async signPresentationRequest(
    ...
    if (!suiteInfo) {
      throw new CredoError(`The requested proofType Ed25519Signature2018 is not supported`)
    }
    ...
    const result:any = await vc.signPresentationRequest({
      presentation: JsonTransformer.toJSON(options.presentationRequest), // I know, we may call this presentationRequest
      suite: suite,
      purpose: options.proofPurpose,
      documentLoader: this.w3cCredentialsModuleConfig.documentLoader(agentContext),
    })

    return new W3cPresentationRequest(result);
  }
  ...
  public async signACPContext(
    ...
    const result = await vc.signPresentation({
      presentation: JsonTransformer.toJSON(options.acpPolicy),
      suite: suite,
      challenge: options.challenge,
      domain: options.domain,
      documentLoader: this.w3cCredentialsModuleConfig.documentLoader(agentContext),
    ...
    return JsonTransformer.fromJSON(result, W3cPresentationRequest)
  }
  ...
  public async signWrappedPresentationRequest(
    ...
    const result:any = await vc.signPresentationRequest({
      presentation: JsonTransformer.toJSON(options.wrappedVPR), // I know, we may call this presentationRequest
      suite: suite,
      purpose: options.proofPurpose,
      documentLoader: this.w3cCredentialsModuleConfig.documentLoader(agentContext),
    })

    return new W3cPresentationRequestWrapper(result);
  }
  ...
  public async verifyWrappedPresentation(
    ...
    try {
      ...
      const verifyOptions: Record<string, unknown> = {
        presentation: JsonTransformer.toJSON(options.wrappedPresentation),

...
import type {
  ...
  W3cVerifyPresentationResult,W3cPresentationRequest
} from '../../../vc'
  ...
  implements ProofFormatService<DifPresentationExchangeProofFormat>
{
  ...
  public async processPresentation(
    ...
    let partialRequestAttachment: any = requestAttachment;
    if (partialRequestAttachment.data.json.type[0] === "VerifiablePresentationRequestWrapper") {
      ...
      requestAttachment = partialRequestAttachment as Attachment
    }
  ...
  public async createWrappedRequest(
      agentContext: AgentContext,
      { attachmentId, proofFormats, requestWrapper }: FormatCreateWrappedRequestOptions<DifPresentationExchangeProofFormat>
  ): Promise<ProofFormatCreateReturn> {
    const format = new ProofFormatSpec({
      format: PRESENTATION_EXCHANGE_PRESENTATION_REQUEST,
      attachmentId,
